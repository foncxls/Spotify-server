    ...
    globalServer = http.createServer(async (req, res) => {
        ...
        if (pathname.startsWith('/stream/')) {
            ...
            try {
                ...
                const pageTitle = artists ? `${title} â€ ${artists}` : title;
  ...
  <script>
    ...
    player.addEventListener('ended', () => {
      ...
      if (currentIndex < queue.length) {
        ...
        document.title = next.artists ? (next.title + ' â€ ' + next.artists) : next.title;
        ...
        if (pathname === '/api/search' || pathname === '/api/search/track') {
            ...
            try {
                ...
                data.forEach(item => { item.stream_url = `${baseUrl}/stream/${item.id}`; });
        ...
        if (pathname.startsWith('/api/track/')) {
            ...
            try {
                ...
                data.stream_url = `${baseUrl}/stream/${data.id}`;
        ...
        if (pathname.startsWith('/api/artist/')) {
            ...
            try {
                ...
                if (data.top_tracks) {
                    data.top_tracks.forEach(t => t.stream_url = `${baseUrl}/stream/${t.id}`);
                }
                return sendJson(200, { status: 'success', data });
            } catch (err) {
                return sendJson(500, { status: 'error', message: err.message });
        ...
        if (pathname.startsWith('/api/album/')) {
            ...
            try {
                ...
                if (data.tracks) {
                    data.tracks.forEach(t => t.stream_url = `${baseUrl}/stream/${t.id}`);
                }
                return sendJson(200, { status: 'success', data });
            } catch (err) {
                return sendJson(500, { status: 'error', message: err.message });
            ...
            try {
                ...
                data.forEach(sec => {
                    sec.items.forEach(item => {
                        item.stream_url = `${baseUrl}/stream/${item.id}`;
                    });
                });
        ...
        return sendJson(200, {
            ...
            status: "online",
            endpoints: {
                search: `${baseUrl}/api/search?q=Shape of My Heart&limit=5`,
                search_artist: `${baseUrl}/api/search/artist?q=Backstreet Boys`,
                search_album: `${baseUrl}/api/search/album?q=Midnights`,
                track: `${baseUrl}/api/track/:track_id`,
                lyrics: `${baseUrl}/api/lyrics/:track_id`,
                canvas: `${baseUrl}/api/canvas/:track_id`,
                artist: `${baseUrl}/api/artist/:artist_id`,
                album: `${baseUrl}/api/album/:album_id`,
                playlist: `${baseUrl}/api/playlist/:playlist_id`,
                home: `${baseUrl}/api/home`,
                stream_player: `${baseUrl}/stream/:id`,
                audio_stream: `${baseUrl}/audio/:id`
            }
        });
    });
